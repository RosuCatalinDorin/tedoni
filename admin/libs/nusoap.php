	...
	function schemaStartElement($parser, $name, $attrs) {
		...
		switch($name){
			...
			case 'complexType':
				...
				if(isset($attrs['name'])){
					//  what is the scope of named complexTypes that appear
	...
	function getTypeDef($type){
		...
		if((! $is_element) && isset($this->complexTypes[$type])){
			...
			if (!isset($this->simpleTypes[$type]['phpType'])) {
				...
				//  can this ever really apply (i.e. what is a simpleType really?)
...
class soap_transport_http extends nusoap_base {
	...
	*/
	function connect($connection_timeout=0,$response_timeout=30){
	  	...
	  	// "regular" socket.
	  	//  disabled for now because OpenSSL must be *compiled* in (not just
		...
		foreach($header_array as $header_line){
			...
			if(count($arr) > 1){
				...
				if ($header_name == 'set-cookie') {
					//  allow multiple cookies from parseCookie
...
//		}
		...
		if ($cErr != '') {
        	...
        	//  there is a PHP bug that can cause this to SEGV for CURLINFO_CONTENT_TYPE
	 ...
	 */
	/*
	 *  allow a Set-Cookie string to be parsed into multiple cookies
	 */
		...
		// returned value(s)
			...
			if($this->wsdl){
				if (sizeof($this->opData['output']['parts']) > 1) {
					...
					//  verify that it is not already wrapped!
...
class wsdl extends nusoap_base {
	...
	function fetchWSDL($wsdl) {
        ...
        // imports
        //  handle imports more properly, grabbing them in-line and nesting them
    	$imported_urls = array();
    	$imported = 1;
    	while ($imported > 0) {
    		$imported = 0;
	...
	*/
	function end_element($parser, $name){ 
		...
		if ($this->documentation) {
			// track the node to which documentation should be assigned; it can be a part, message, etc.
	...
	*/
			...
			if (is_array($parameters)) {
				...
				if ($style == 'document' && $use == 'literal' && $part_count == 1 && isset($parts['parameters'])) {
					...
					if ($direction == 'output' && $parametersArrayType == 'arraySimple' && $parameter_count == 1) {
						//  consider checking here for double-wrapping, when
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
			...
			if($ns == $this->XMLSchemaVersion || $ns == 'http://schemas.xmlsoap.org/soap/encoding/'){
				...
				if (is_null($value)) {
					if ($use == 'literal') {
						...
						//  depends on nillable, which should be checked before calling this method
		...
		if ($phpType == 'struct') {
			...
			if (is_null($value)) {
				if ($use == 'literal') {
					//  depends on minOccurs and nillable
				...
				if (isset($optionals)
					...
					if (isset($attrs['maxOccurs']) && ($attrs['maxOccurs'] == 'unbounded' || $attrs['maxOccurs'] > 1) && isset($v) && is_array($v) && $this->isArraySimpleOrStruct($v) == 'arraySimple') {
						...
						if (is_null($v) && isset($attrs['minOccurs']) && $attrs['minOccurs'] == '0') {
							...
							//  serialize a nil correctly, but for now serialize schema-defined type
						    $xml .= $this->serializeType($eName, isset($attrs['type']) ? $attrs['type'] : $attrs['ref'], $v, $use, $encodingStyle, $unqualified);
...
class nusoap_client extends nusoap_base  {
	...
	* @access   public
	*/
	function call($operation,$params=array(),$namespace='http://fcubs.ofss.com/service/FCUBSCustomerService',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
