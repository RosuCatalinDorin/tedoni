...
//echo 'aaaaaaaaaa';
$ch = curl_init();
$url=$url='http://btclient/sites/mainsite/_vti_bin/BT.iQuest.ClientEnrollment/SignatureClient.svc/web/Get'.$_REQUEST[type].'/'.$_REQUEST[cif];
...
if(!curl_errno($ch)) {
   ...
   // print_r_tree($data);
    echo "<table border='1'>";
    $data=$data['Client']['Beneficiaries']['Beneficiary'];
    foreach($data as $v){
        if ($v[Right]!='DELEGAT'){
            echo "<tr>";
            echo "<td>$v[CIF]</td>";
            ...
            echo "<td>"?> <img src="data:image/<?php echo $img[Type]?>;base64,<?php echo $img[Binary]?>" style="height: 100px" /><?php echo "</td>";
            echo "</tr>";
        }
